...
const ReviewsSection = ({ cards4, deleteRating }) => {
    ...
    const settings = {
        dots: true,
        infinite: cards4.length > slidesToShow,
        speed: 500,
        centerMode: cards4.length >= 3,
        centerPadding: "0px",
    ...
    return (
        <section className={styles.reviews} style={{ padding: "40px 20px", textAlign: "center" }}>
            <div style={{
                maxWidth: deleteRating && isSmallScreen ? "500px" : "",
                width: "100%",
                margin: deleteRating || isSmallScreen ? "0 auto" : "",
            }}>
                <div className={styles.mainTitle}>
                    <h3>Отзывы</h3>
                </div>
                <h2 className={styles.title}>Что о нас говорят пользователи <br /> и как оценивают наш сервис?</h2>
                <p className={styles.subTitle}>Больше живых отзывов можно найти в нашем Telegram канале <Link style={{color:"#989899"}} href={'https://t.me/+ASvlNwHpbGg5MGYy'} target="_blank"><span className="click" style={{ cursor: "pointer" }}>@AbbsNet</span></Link></p>
            </div>
            {
                !deleteRating && (
            ...
            <Swiper
                ...
                style={{ maxWidth: '1060px', padding: '0 60px', overflow: 'visible' }}
            >
                {cards4.map((user, index) => (
            ...
            <div className={styles.buttonItem}>
                <Link href={'https://t.me/+ASvlNwHpbGg5MGYy'} target="_blank">
